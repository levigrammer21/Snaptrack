...
function bindBoxscoreSort(){
  $$('#boxBody .stat-sort').forEach(th=>{
    const sort=()=>{
...
function offDesc(pl){if(pl.type==='snap')return 'Snap only'; const r=roster(state.current.team); const p=id=>r.find(x=>x.id===id); const outcomes=[(pl.touchdown||pl.result==='td')?'TD':'',pl.fumble?'FUMBLE':''].filter(Boolean).join(' · '); if(pl.type==='pass')return `Pass ${p(pl.primary)?.name||''} → ${p(pl.receiver)?.name||''} ${pl.yards||0} yds ${pl.result||''} ${outcomes}`.trim(); return `${pl.type} ${p(pl.primary)?.name||''} ${pl.yards||0} yds ${outcomes}`.trim();}
